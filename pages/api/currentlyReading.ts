...
const currentlyReadingApi = async (
  req: NextApiRequest,
  res: NextApiResponse
) => {
  const { GOODREADS_KEY, GOODREADS_USER_ID } = process.env

  const readingShelf = await fetch(
    `https://www.goodreads.com/review/list/${GOODREADS_USER_ID}.xml?key=${GOODREADS_KEY}&v=2&shelf=currently-reading&sort=date_read`
  )
...
export default currentlyReadingApi
